...
export function FisioAI({ onNavigate }: { onNavigate: (v: PatientView) => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <header className="bg-surface px-4 sm:px-8 py-4 flex items-center gap-3 sm:gap-4 border-b border-outline-variant/20 sticky top-0 z-30 shrink-0">
        <button onClick={() => onNavigate('dashboard')} className="p-2 rounded-lg hover:bg-surface-container shrink-0">
          <span className="material-symbols-outlined text-on-surface-variant">arrow_back</span>
        </button>
        <span className="material-symbols-outlined text-primary shrink-0" style={{ fontSize: 24 }}>smart_toy</span>
        <h1 className="text-base sm:text-lg font-semibold text-primary truncate">Asistente IA - Fisio</h1>
        <div className="flex-1" />
        <button
          onClick={() => alert('Notificaciones: No hay nuevas notificaciones')}
          className="p-2 rounded-lg hover:bg-surface-container shrink-0"
        >
          <span className="material-symbols-outlined text-on-surface-variant" style={{ fontSize: 22 }}>notifications</span>
        </button>
      </header>

      <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 p-4 sm:p-6 flex-1 overflow-hidden">
        {/* Chat Container */}
        <div className="flex-1 flex flex-col glass-card rounded-3xl overflow-hidden min-h-0">
          {/* Chat Feed */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 sm:p-8 flex flex-col gap-4 sm:gap-6 chat-scroll min-h-0">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-3 sm:gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 ${
                  ...
                  <span
                    ...
                    style={{ fontSize: 20, fontVariationSettings: msg.role === 'assistant' ? "'FILL' 1" : "'FILL' 0" }}
                  >
                ...
                {/* Message bubble */}
                <div className={`p-4 sm:p-5 shadow-sm whitespace-pre-wrap max-w-[calc(100%-3.5rem)] sm:max-w-2xl ${
                  msg.role === 'assistant'
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 mt-2">
                {SUGGESTIONS.map((s) => (
                  <button
                    ...
                    className="flex flex-col items-start p-3 sm:p-4 bg-white border border-outline-variant rounded-xl hover:border-primary hover:bg-primary/5 transition-all text-left group"
                  >
            ...
            {loading && (
              <div className="flex gap-3 sm:gap-4">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-primary flex items-center justify-center shrink-0">
                  <span className="material-symbols-outlined text-white animate-pulse" style={{ fontSize: 20, fontVariationSettings: "'FILL' 1" }}>smart_toy</span>
                </div>
                <div className="bg-white border border-outline-variant rounded-r-2xl rounded-bl-2xl p-4 sm:p-5 shadow-sm">
          ...
          {/* Input Area */}
          <div className="p-4 sm:p-6 bg-white/50 border-t border-outline-variant/30 shrink-0">
            <form onSubmit={handleSubmit} className="relative flex items-center gap-3">
              <div className="flex-1 relative">
                <input
                  ...
                  disabled={loading}
                  className="w-full bg-white border border-outline-variant rounded-full py-3 sm:py-4 pl-5 sm:pl-6 pr-14 focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none text-sm disabled:opacity-50"
                />
                <button
                  type="submit"
                  disabled={loading || !input.trim()}
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-10 sm:h-10 bg-primary text-white rounded-full flex items-center justify-center hover:brightness-110 active:scale-95 transition-all disabled:opacity-50"
                >
                  <span className="material-symbols-outlined" style={{ fontSize: 20 }}>send</span>
                </button>
              </div>
            </form>
            <p className="text-[10px] text-center text-on-surface-variant mt-3 sm:mt-4 uppercase tracking-widest" style={{ fontFamily: 'JetBrains Mono, monospace' }}>
        ...
        <aside className="w-80 flex-col gap-6 hidden lg:flex shrink-0">
          {/* Disclaimer Card */}
          <div className="bg-primary text-white rounded-3xl p-6 shadow-md relative overflow-hidden">
            ...
            <span className="material-symbols-outlined absolute -bottom-4 -right-4 text-white/10 rotate-12" style={{ fontSize: 96 }}>spa</span>
          </div>
